...
import { getUserRepo,createUserRepo,clearTokenDetails } from "../model/user.repo.js";
...
export const registerUser = async (req, res, next) => {
    ...
    try {
        ...
        if (!name || !email || !password)
            return res.send({ status:false, message: "All the fields are mandatory" });
        ...
        return res.send({status:false,message:"unautherized behaviour. Please try it again"});
    }
};
...
export const resetPassword=async(req,res,next)=>{
    try{
        ...
        if(!user.password) res.send({status:false,message:"You have register with us using Google. Try to login with the same"});
...
export const setNewPassword=async(req,res)=>{
    try{
        ...
        if(password!=confirmPassword){
             res.send({status:false,message:"New Password and Confirm Password don't match"});
        }
        else{
            const user=await getUserRepo({email});
...
export const verifyOtp=async(req,res)=>{
    ...
    if(!email || !token) res.send({status:false,message:"Enter otp carefully"});

    const user=await getUserRepo({email});

    if(!user) res.send({status:false,message:"User not found"});

    if(user.resetPasswordExpire<Date.now()) res.send({status:false,message:"OTP is Expired. Try Again"});

    if(token!=user.resetPasswordToken) res.send({status:false,message:"OTP didn't match"});
